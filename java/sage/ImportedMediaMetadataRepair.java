...
final class ImportedMediaMetadataRepair
{
  ...
  {
    // Valid imported MKVs already play correctly through an unmodified stock
    // SageTV server. Keep this database-repair escape hatch opt-in so ordinary
    // playback never mutates metadata merely because a newer Core is present.
    ...
    if (reinitialized)
    {
      ContainerFormat repairedFormat = mediaFile.fileFormat;
      System.out.println("Imported media metadata repair complete: file=" + mediaPath +
          " duration=" + mediaFile.getDuration(0) + " format=" + repairedFormat);
    }
      ...
      long formatDuration, int streamCount)
  {
    ...
    // A completed playable video cannot genuinely be one millisecond long.
    // Require corroborating invalid format metadata so unusual tiny non-video
    // imports are never reparsed by this playback-only safety net.
